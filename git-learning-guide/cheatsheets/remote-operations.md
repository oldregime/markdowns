...
- **Commit B**: A commit on a separate branch for bug fixes.
- **Commit C**: A commit on a feature branch that diverged from the main branch.
...
3. **Build the Project**
   Use your build tool (e.g., Webpack, Gulp) to create the production build.
...
Historically, the default branch in Git repositories was named **master**. However, many organizations, including GitHub, have transitioned to using **main** as the default branch name to promote inclusive language.
